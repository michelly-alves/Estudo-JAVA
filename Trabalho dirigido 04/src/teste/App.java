	...
	public static void main(String[] args) throws ParseException {
		try (Scanner scan = new Scanner(System.in)) {
			...
			for (i = 0; i < N; i++) {
				...
				if(vet[i].validaDataAdmissao(vet, N) == false) {
					System.out.println("Data de admissão incorreta!");
				}
				if(vet[i].validaDataNascimento(vet[i].getDataNascimento(), vet, N) == false) {
					System.out.println("Data de nascimento incorreta!");
				}
				if(vet[i].verificaFuncionariosIguais(vet[i].getNome(), vet[i].getCpf(), vet[i].getSexo(), vet, N) == false){
					System.out.println("Funcionário igual a outro já cadastrado!");
				}
				System.out.println("");

			}
			vet[0].calculaIdade(vet[0].getDataNascimento(), vet, N);
